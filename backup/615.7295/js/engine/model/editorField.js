...
function eEditorField(name, data) {
    ...
    self.init = f(){
        ...
        self.data = mergeObjects({
            ...
            wrap: f(self, data, is_readonly) {
                var fullSet = cr('div','line');
                var label = fullSet.cr('div','key');
                var info = fullSet.cr('div','info');
                var input;
                if ( is_readonly ) {
                    input = self.data.buildView(self, data);
                } else {
                    input = self.data.build(self, data);
                }
    ...
    self.getInput = f(data, is_readonly) {
        var newData = mergeObjects(self.data, data);
        var nodes = newData.wrap(self, data, is_readonly);
