...
//--- OUTER INTERFACE --- 
    ...
    self.showNew = f(obj) {
        ...
        newData.onshow = self.data.onshow;
...
ED2Q.processors = {
    ...
    update: f(editor, data) {
        ...
        editor.submit = f(obj, basedata, langdata, onfinish) {
            ...
            ORM.req((obj._bid||obj._oid)+':update', basedata, function() {
                if ( okeys(langdata).length == 0 ) { q.doNext(); }
...
function prepEditor2(model, types, dataSet) {
    map(parseLS(types), f(type) {
        ...
        map(parseLS('inherit,level,schema,lschema,group,order,onshow,readonly'), f(key) {
