...
SYS.dictView = {};
CONF.project.modelVisName = CONF.project.modelVisName || {};
...
var ORM = new eStorage('orm', {
    ...
    req: function(self, path, data, todo, params) {
        ...
        self.respFuncParse = self.respFuncParse || f(resp) {
        ...
        var respFunc = function(resp, fullData) {
            self.respFuncParse(resp, fullData);
            todo(resp[okeys(resp)[0]], resp, fullData);
        };
    ...
    _getViewDict: function(self, name, func) {
        ...
        var url = ['/_view',PAGE.lang,name,('?v='+(new Date()*1))].join('/');
        
        //-log(url);
        
        getRawData(url, function(data) {
            var t = cr('div');
            t.innerHTML = data;
            var result = {};
            var oids = [];
    ...
    storePriv: function(self, model, id, data) {
        ...
        var emitAddr = ['ORM',model,id];
        if ( lang ) { emitAddr.push(lang) }
        EVENT.emit(emitAddr.join('.'), data);
...
}, ['req', 'getViewDict', 'toUrl', 'diff', 'lang', 'normaliseForUpdate','prep','getVisName','getFullName','onModel','remOnModel','_copyDictView','rel','getDropdownMap','ID']);
