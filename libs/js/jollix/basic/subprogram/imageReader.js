...
function getFileName(file) {
    return file.name || file.fileName || (new Date()*1)+'.ext';
}


function resizeImage(file, dims, func) {
    var resizeType = Math.max(dims.split('x')[2]||1, 1);
    var name = fileName(file)||(new Date()*1).toString();
...
function createImage(mediatypeName, file, dims, func) {
    ...
    getRawData(['/_view/',PAGE.lang,'/byname_mediatype/',mediatypeName,'/'].join(''), f(dt) {
        PROTOCOL.api.read(parseObj(dt),null, f(objd){
            var objs = objd.mediatype;
            ORM.respFuncParse(objd);
            var mediatype = objs[0];
