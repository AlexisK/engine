...
new eProtocol('api', {
    ...
    write: function(self, reqMap,reqData,funcOk,funcBad,options) {
        ...
        var ch = 1;
        if ( reqDict.method == 'select' && okeys(reqDict.selector).length == 0 ) {
            if ( SYS.dynaview_select && SYS.dynaview_select.contains(reqDict.table) ) {
                ch = 0;
                getRawData(['/_dynaview/',(PAGE.lang || 'en'),'/select_',reqDict.table,'/'].join(''),function(dt) {
                    //-log(parseObj(dt));
                    self.read(parseObj(dt),null, funcOk, funcBad);
                })
            } else if ( SYS.view_select && SYS.view_select.contains(reqDict.table) ) {
                ch = 0;
                getRawData(['/_view/',(PAGE.lang || 'en'),'/select_',reqDict.table,'/'].join(''),function(dt) {
                    //-log(parseObj(dt));
                    self.read(parseObj(dt),null, funcOk, funcBad);
                })
            }
    ...
    read: function(self, resp, respObj, funcOk, funcBad) {
        ...
        if ( status == 'OK' ) {
            ...
            if ( PAGE.level >= %levelManager ) { SYS.notify([resp.statusData.field, resp.statusData.exception].join(' '), 'center red'); }
            funcBad(resp);
        }
    }
