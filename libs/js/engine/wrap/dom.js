...
function nodeval(node, raw) {
    if ( node.__isHtml ) { return node.innerHTML; }
    if ( !raw ) {
        var vv = node.validval;
        if ( vv ) { return vv; }
    }
    if ( def(node.value) ) {
...
function domSearch(list, pattern, funcOk, funcBad, params) {
    ...
    if ( !def(params.limit) ) { params.limit = list.length;}
...
function getInDocument(node) {
    if ( node == document ) { return true; }
    if ( !node ) { return false; }
    return getInDocument(node.parentNode);
}
