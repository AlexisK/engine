...
function evt(obj, events, todo) {
    if ( !def(obj) ) { return null; }
    events = parseLS(events||'');
    map(events, f(event) {
        obj.addEventListener(event, todo);
    });
}

function evtDel(obj, events, todo) {
    if ( !def(obj) ) { return null; }
    events = parseLS(events||'');
    map(events, f(event) {
...
//- FF .sort(true) workaround
Array.prototype._jsort = Array.prototype.sort;
Array.prototype.sort = function() {
    try {
        return Array.prototype._jsort.apply(this, arguments);
    } catch(err) { return this._jsort(); }
}
