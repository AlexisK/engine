...
//--- INIT --- 
    ...
    self._init = function() {
        ...
        self.form_lang_selector = data.form_lang_selector || f(self, lang) {
            var ro = {
                id: ['=',self.obj.id]
            };
            if ( lang ) {
...
//--- BUILD --- 
    self._buildBasis = f() {
        ...
        self.V.custom = self.dom.V.block.cr('div');
        ...
        self.dom.C.onLang = f(lang) {
            ...
            if ( self.obj ) {
                if ( self.langPrefix ) {
                    ...
                    ORM.req([self.obj._model,'select'].join(':'), f(list) {
                        var newObj = list[0];
                        if ( newObj ) {
                            self.obj = newObj;
                            self.setData();
                        ...
                        selector: self.form_lang_selector(self, lang)
                    });
                } else {
                    var frng = RNG(self.inputs);
    ...
    self._buildLine = f(keyMap, title, schema, target, isLang){
        ...
        var INP = edField.getInput(mergeObjects(self.editordata, schema), self.readonly);
    ...
    self.build = f() {
        ...
        var bckp = {
            schema: DCO(self.schema),
            lschema: DCO(self.lschema),
            group: DCO(self.group)
        };
...
//--- DATA WORK --- 
    ...
    self.getLangObj = f() {
        if ( self.langPrefix ) {
            return $AD(self.obj, [self.langPrefix,self.lang].join('.'), {
                autocreate:{}
            });
        }
        return self.obj;
...
//--- OUTER INTERFACE --- 
    ...
    self.showNew = f(obj) {
        var newData = DCO(self.data);
        //-ne//-wData.onshow = self.data.onshow;
        //-newData.schema = data.schema;
        //-newData.lschema = data.lschema;
    ...
    self._show = f(obj) {
        ...
        self.data.onshow(self.V.custom, obj, self);
    }
    ...
    self._submit = f() {
        self._dovalidate(f(){
            ...
            if ( self.langPrefix ) {
                ...
                self.updRec(upd, self.langPrefix.split('.'), self.obj);
                
                var updlp = $AD(upd, self.langPrefix, {
                    autocreate: {}
                });
                
                mapO(ORM.model.lang, f(lang) {
                    updlp[lang.name] = mergeObjects($AD(self.obj, [self.langPrefix, lang.name].join('.'), {
                        autocreate: {}
                    }), self.lupd[lang.name]);
                });
                lupd = {};
...
//--- REWRITE FUNCTIONS --- 
    self.rewritekeys_old = parseLS('schema,lschema,defaultObject,group,order,_getButton,getobject,submit,validate,onSetData');
    self.rewritekeys = parseLS('_getButton,getobject,submit,validate,onSetData');
...
ED2Q.processors = {
    ...
    insert: f(editor, data) {
        ...
        editor._getButton = f(self) {
            ...
            node.attach(cr('span').VAL([PAGE.ld(editor.selfModel)].join(' ')));
            
            node.onclick = f() {
                var o = DCO(editor.defaultObject||{});
        //-        //-log(o);
                editor.showNew(o);
            }
        ...
        editor.submit = f(obj, basedata, langdata, onfinish) {
            ...
            if ( data.data_proxy ) {
                data.data_proxy(obj, basedata, langdata);
            }
            ...
            var method;
            try {
                method = data.cast_method.insert;
            } catch(err) {}
            method = method || 'insert';
            
            ORM.req(editor.selfModel+':'+method, basedata, function(list) {
                if ( list.length == 0 ) {
    ...
    update: f(editor, data) {
        ...
        editor.submit = f(obj, basedata, langdata, onfinish) {
            ...
            var method;
            try {
                method = data.cast_method.update;
            } catch(err) {}
            method = method || 'update';
            
            ORM.req((obj._model)+':'+method, basedata, function() {
                if ( okeys(langdata).length == 0 ) { q.doNext(); }
                ...
                mapO(langdata, function(data, lang) {
                    q.add(function(done) {
                        if ( okeys(data).length == 0 ) {
                            ...
                            ORM.req([obj._model,method].join(':'), data, done, {
                                selector: editor.form_lang_selector({obj:obj}, ORM.O('lang_'+lang))
                            });
                        }
                    });
                ...
                selector: editor.form_lang_selector({obj:obj}, ORM.O('lang_'+obj.lang_id))
            });
    ...
    view: f(editor, data) {
        ...
        editor._getButton = f(self) {
            var node = cr('div', 'asBtn fa');
            node.attach(SVG.views().attr({class:'svg'}));
            node.attach(cr('span').VAL(PAGE.ld('view')));
...
function prepEditor2(model, types, dataSet) {
    map(parseLS(types), f(type) {
        var data = DCO(dataSet);
        //-data.schema = dataSet.schema;
        //-data.lschema = dataSet.lschema;
        //-data.objschema = dataSet.objschema;
        //-data.onshow = dataSet.onshow;
        data.view_readonly = true;
        
        map(parseLS('cls,inherit,level,schema,lschema,group,order,onshow,readonly,data_proxy'), f(key) {
