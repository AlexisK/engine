...
function ePop(popName, cls, data, parent) {
    ...
    self.initDom = function() {
        ...
        self.block.onmousedown = f() {
            if ( !self.isBlocked ) {
                self.C.toclose = true;
            }
        }
        self.block.onmouseup = f() {
            if ( self.C.toclose ) {
        ...
        self.V.cont.onmousedown = f() {
            tm(f() { self.C.toclose = false; });
        }
        self.V.cont.onmouseup = f() {
            self.C.toclose = false;
        }
    ...
    self.show = function(node, persistent, rdata) {
        ...
        if ( ENGINE.popMap.length == 1 ) {
            document.body.addCls('noScroll');
        }
    ...
    self.hide = function(params) {
        ...
        if ( self.C.contElem ) {
            ...
            if ( ENGINE.popMap.length == 0 ) {
                document.body.remCls('noScroll');
            }
