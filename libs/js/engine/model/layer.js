...
SYS.globalUrlModification = f(url){return url;}
    ...
    self.go = function(url, func, params) {
        ...
        if ( !url ) {
            ...
            self.data.getData(self, self.urlData.url, function() {
                ...
                self.data.ontravel(self, params);
...
function layerManager() {
    ...
    self.go = function(url, func, params) {
        //-log('go LM',url);
        url = SYS.globalUrlModification(url||window.location.href);
        params = params||{};
    ...
    self.fetchPos = function(func) {
        ...
        if ( layer.data.parent ) {
            ...
            var parentUrl = SYS.globalUrlModification(layer.data.parentUrl(window.location.href));
            var params = {noHistory:true,noHide:true,is_fetchPos:true};
            
            parent.go(parentUrl, function() {
                layer.data.ontravel(layer, params);
                func();
            }, params)
        } else {
            func();
