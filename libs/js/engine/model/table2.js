...
ENGINE.tableHistory = parseObj(sessionStorage.getItem('tbl2History')||'[]');
...
function eTable2(name, data) {
    ...
    self.init = function() {
        ...
        if ( def(data.prep) && data.level <= PAGE.level ) {
        ...
        var editors = EDITOR2[self.data.model];
        if ( editors ) {
            self.edFuncORM = function(obj, fn) {
                
                if ( editors.update ) {
                    var edBtn = editors.update.getButton();
                    edBtn.ref = obj._oid;
                    fn(edBtn);
                } else if ( editors.view ) {
                    var vwBtn = editors.view.getButton();
                    vwBtn.ref = obj._oid;
                    fn(vwBtn);
                }
                if ( editors.delete ) {
                    var delBtn = editors.delete.getButton();
                    delBtn.ref = obj._oid;
                    delBtn.ondelete = self.rebuild;
                    fn(delBtn);
                }
            }
            
        } else {
            var ref = EDITOR[self.data.model];
            if ( ref ) {
                self.edFuncORM = function(obj, fn) {
    ...
    self.buildModelZone = function() {
        if ( EDITOR2[self.data.model] ) {
            if ( EDITOR2[self.data.model].insert ) {
                var btn = EDITOR2[self.data.model].insert.getButton();
    ...
    self._buildPaginator = function() {
        self.pbPrev  = self.block.V.header.cr('div', 'asBtn arr prev').VAL(PAGE.ld('prev'));
        self.pbPages = self.block.V.header.cr('div', 'pageCont');
        self.pbNext  = self.block.V.header.cr('div', 'asBtn arr next').VAL(PAGE.ld('next'));
    ...
    self.getBlock = function() {
        ...
        self.block._onpopclose = f() {
            sessionStorage.removeItem('tbl2Shown');
        }
        return self.block;
    }
    ...
    self.show = function(rdata) {
        ...
        ENGINE.tableHistory.remove(name);
        ENGINE.tableHistory.add(name);
        sessionStorage.setItem('tbl2History', parseStr(ENGINE.tableHistory));
        tm(f(){ sessionStorage.setItem('tbl2Shown', name); }, 100);
...
TVIEW.rel      = function(       obj, key) { var km = key.split('_'); return ORM.getFullName(ORM.rel(obj, km.sl([0,-1]).join('_'))); }
//- project/conf/tableTypes


TVIEW.boolReadonly = f(obj, key) {
    var newNode = cr.bool().VAL(obj[key]||false);
    clearEvents(newNode);
    newNode.addCls('readonly');
    return newNode;
}
...
TINP._dropdown = f(self, field, block) {
    block.val = 'None';
...
TINP.dropdown = function(self, field, dd) {
    ...
    return TINP._dropdown(self, field, block);
}

TINP.modelDropdown = function(self, field, key) {
    var block = cr.ormdropdown(RNG(ORM.model[key]));
    return TINP._dropdown(self, field, block);
}


//- inbill, 'currency', 'coffer', 'currency'
TINP.modelJoinDropdown = function(self, field, key, viewKey, kk, vkk) {
    kk  = kk  || (key + '_id')
    vkk = vkk || (viewKey + '_id');
    var viewDMap = ORM.getDropdownMap(viewKey);
    var prep1 = {};
    var resultDict = {};
    
    mapO(ORM.model[key], f(obj) {
        prep1[obj[vkk]] = prep1[obj[vkk]] || [];
        prep1[obj[vkk]].add(obj.id);
    });
    
    mapO(prep1, f(ids, parentId) {
        resultDict[ids.join(',')] = viewDMap[parentId];
    });
    
    resultDict['None'] = '?';
    var block = cr.dropdown(resultDict);
    block.val = 'None';
    ...
    block.onupdate(function(val) {
        if ( val == 'None' ) {
            delete self.selectBy[kk];
        } else {
            var rds = val.split(',');
            var rd = [];
            map(rds, f(d) { rd.push(parseInt(d)); });
            
            if ( rd.length == 1 ) {
                self.selectBy[kk] = ['=',rd[0]];
            } else {
...
TINP.bool4 = function(self, field) {
    var block = cr.switcher({
        'none':'Any',
        'true':'True',
        'false':'False',
        'null':'None'
    });
    block.states.none.addCls('blue');
    block.states.true.addCls('green');
    block.states.false.addCls('red');
    
    block.onupdate(f() {
        if ( block.val == 'none' ) {
            delete self.selectBy[field];
        } else if ( block.val == 'null' ) {
            self.selectBy[field] = ['!=',true,'!=',false];
        } else if ( block.val == 'true' ) {
            self.selectBy[field] = ['=',true];
        } else {
            self.selectBy[field] = ['=',false];
        }
        self.rebuild();
    });
    ...
    block.setVal = function(val) { 
        if ( val[0] == '=' ) {
            if ( val[1] ) { block.val = 'true'; } else { block.val = 'false'; }
        } else {
            block.val = 'null';
        }
...
TINP.rangeNumber = function(self, field) {
    ...
    block.inp1.onupdate(f(){block._doFilter();});
    block.inp2.onupdate(f(){block._doFilter();});
