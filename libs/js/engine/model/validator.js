...
function eValidator(name, data, isCustom) {
    ...
    self.runSingle = function(elem, func) {
        if ( func(elem.val) ) {
            ...
            addCls(elem, 'isValid');
            if ( elem.V && elem.V.viewNode ) {
                remCls(elem.V.viewNode, 'notValid');
                addCls(elem.V.viewNode, 'isValid');
            }
            return true;
        } else {
            remCls(elem, 'isValid');
            ...
            if ( elem.V && elem.V.viewNode ) {
                remCls(elem.V.viewNode, 'isValid');
                addCls(elem.V.viewNode, 'notValid');
            }
            return false;
        }
    }
    ...
    self.run = function(target, func) {
        ...
        evt(target, 'change', f(ev) {
            self.runSingle(this, func);
        });
