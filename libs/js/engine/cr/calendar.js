...
cr.dateinput = function(cls, parent) {
    ...
    self.C.rules = {
        dd: {
            customInput: true,
            noUpdateOnVal: true,
            validator: f(val) { var iv = parseInt(val); return (iv == val) && iv >0 && iv <= 31; }
        },
        mm: {
            limit: 12,
            noUpdateOnVal: true
        ...
        yy: {
            customInput: true,
            noUpdateOnVal: true,
            validator: f(val) { var iv = parseInt(val); return (iv == val) && iv > 1900 && iv <= new Date().getFullYear(); }
        }
    };
    self.F._prepData();
    ...
    map(['dd','mm','yy'], function(key) {
       self.V[key] = cr.dropdown(self.C[key], null, self, self.C.rules[key]);
