new eProcessor('viewcalc', {
    ...
    _process: function(self, db) {
        ...
        self.V.htmlstrings = S('.mk_fetchstrings', self);
        self.needEmpty = [];
        ...
        self.strNodes = {
            ...
            'available' : S('.mk_availableString', self)
        }
        ...
        self.F.ito = function() {
            ...
            var calcData = calcObj.calcExchange(self.current.fetchexchange, {
                in_sum: val,
        ...
        self.F.setExchange = function(exchange) {
            self.C.insertData = {};
        ...
        self.F.drawCustomFields = f() {
            ...
            if ( self.V.in_psi ) {
                ...
                //- sht part 
                if ( self.current.exchange._rel.in_currency != self.current.in_cur || self.current.exchange._rel.out_currency != self.current.out_cur ) {
                    self.V.in_psi.addCls('hidden');
                    self.V.out_psi.addCls('hidden');
                    return 0;
                } else {
                    self.V.in_psi.remCls('hidden');
                    self.V.out_psi.remCls('hidden');
                }
                //- sht part end 
                ...
                if ( (self.current.exchange.params.is_userdata_in || self.current.in_cur.params.is_userdata_in || self.current.out_cur.params.is_userdata_out)  && self.C.psiRule ) {
        ...
        self.F.setCur = function(cur, prefix, changeSum) {
            ...
            if ( cur._rel.paysystem != self.current[prefix+'_ps'] ) {
                self.F.setPs(cur._rel.paysystem, prefix);
            }
            ...
            //-}
            //-
            self.F.calcExch(null, changeSum);
        ...
        self.F.calcExch = function(exc, recalcSum) {
            self.F.fetchActual();
            ...
            self.current.rawexchange = self.C.conf.calc.rawexchanges.filter({
                in_currency_id  : self.current.in_cur.id,
                out_currency_id : self.current.out_cur.id
            })[0];
            
            self.current.fetchexchange = exch || self.current.rawexchange;
            
            
            
            if ( exch ) {
                self.availableExchange = true;
                self.V.in_input.remattr('disabled');
                self.V.out_input.remattr('disabled');
                self.current.exchange = exch;
                
            } else {
                self.availableExchange = false;
                self.V.in_input.attr({disabled:'true'});
                self.V.out_input.attr({disabled:'true'});
            }
            
            if ( self.current.fetchexchange ) {
                self.rate = optDelim(self.current.fetchexchange.rate.toDec());
                self.F.ito();
                self.F.oti();
                self.F.setUrl();
            }
            ...
            if ( recalcSum && self.current.fetchexchange ) {
                if ( self.current.fetchexchange.rate < (1).fromDec() ) {
                    self.F.ito.ENV({
                        ito_val:(1).fromDec()
                    })();
                    self.F.oti();
            ...
            } else if ( self.current.fetchexchange ) {
                self.F.ito();
                self.F.oti();
            }
            self.F.setUrl();
        ...
        self.F.setUrl = function() {
            if ( self.D.burl ) {
                ...
                if ( self._lastUrl != url ) {
                    ...
                    self._lastUrl = url;
                    self.F.fetchUrlData(url);
                }
            }
        }
        
        self.F.fetchUrlData = CEF(function(furl) {
            if ( self.V.htmlstrings.length ) {
                getRawData(furl, f(html){
                    var t = cr('div');
                    t.innerHTML = html;
                    var nodes = S('.mk_fetchstrings', t);
                    map(self.V.htmlstrings, f(bnode, ind) {
                        var fnode = nodes[ind];
                        if ( fnode ) {
                            bnode.innerHTML = fnode.innerHTML;
                        }
                    });
                });
            }
        ...
        self.F.fetchStrs = function() {
            ...
            var ballanceStr;
            var availableStr;
            if( self.availableExchange ) {
                availableStr = '';
                strRate      = [dispRate[0].toLen(2),str_in_name,'=',dispRate[1].toLen(2),str_out_name].join(' ');
                if ( self.V.str_availability_small ) {
                    self.V.str_availability_small.val = PAGE.ld('calcDoExchange');
                }
                self.V.submit.remCls('exchange-blocked');
            } else {
                availableStr = PAGE.ld('Exchange is not available');
                strRate      = '-';
                if ( self.V.str_availability_small ) {
                    self.V.str_availability_small.val = PAGE.ld('calcExchangeBlocked');
                }
            ...
            map(self.strNodes.available, function(node) { node.val = availableStr; });
...
//-                    self.C.conf.calc[prefix+'_cur'].filter({paysystem_id:ps.id}),
        ...
        self.F._drawPsWorker = function(ps, func, row) {
            ...
            node.clickOn = function() {
                //-alert('click ps');
                func(ps);
            };
            node.onclick = node.clickOn;
            node.ontouchstart = node.clickOn;
        ...
        if ( def(self.D.exchange) ) {
            var exch = ORM.O('exchange_'+self.D.exchange);
            
            if ( exch ) {
                self.F.setExchange(exch);
            } else {
                self.F.setExchange(self.C.conf.calc.exchanges[0]);
            }
            self.F.setCur(ORM.O('currency_'+self.D.cur_in),'in');
            self.F.setCur(ORM.O('currency_'+self.D.cur_out),'out');
...
//-        self.F.oti();
        ...
        if ( self.V.swap_btn ) {
            self.V.swap_btn.onclick = f() {
                var in_p = self.current.in_ps;
                var out_p = self.current.out_ps;
                var in_c = self.current.in_cur;
                var out_c = self.current.out_cur;
                
                self.F.setPs(out_p, 'in');
                self.F.setPs(in_p, 'out');
                self.F.setCur(out_c, 'in');
                self.F.setCur(in_c, 'out', true);
            }
        }
        ...
        clearEvents(self.V.submit).onclick = function() {
            ...
            if ( self.D.pop == 'true' )  {
                ...
                if ( this.className.split(' ').contains('exchange-blocked') ) {
                    return false;
                }
