...
new eProcessor('gallery', {
    process: function(self, db) {
        ...
        if ( nodes.length == 0 ) {
            ...
            container.attr('data-isfull', img.D.isfull); //- is used by fullImg2 processor

            container.attr('data-src', img.D.basic);
            img.remattr('width');
            img.remattr('height');
            
            PROCESSOR.fullImg2.process(container);
        ...
        map(nodes, function(img) {
            ...
            if ( def(img) && def(img.src) && def(img.D) ) {
                ...
                var obj = {
                    ...
                    preview: img.D.preprop, //- display uncropped pictures
                    full:    img.D.basic
                };
                db.images.push(obj);
            }
